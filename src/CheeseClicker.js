...
export default function CheeseClicker() {
  ...
  //VALUE: value of each click on button
  ...
  //THIS IS CHEESE PER SECOND, THE FUNCTION
  useEffect(() => {
    const interval = setInterval(() => {
      if (cps > 0) {
        setTotal(total + cps / 2);
      }
    }, 500); // It is set to a half second to appear faster, cps is divided by 2 to compensate
  ...
  return (
    <div className="App">
      {/* ------------- HEADER ------------ */}
      <h1 className="h2">Venture Capitalist but for Rats</h1>

      {/*-------- DISPLAY TOTAL: CHEESE VAULT -------*/}
      <h2 className="h2">
        Cheese Vault: {Math.round(total)}
        <div font-size="smaller">{cps} ch/sec</div>
      ...
      <div className="upBody">
        ...
        <Upgrade
          ...
          desc="The next level of military technology, worth every tax dollar!"
          cps={counts[8] * 260000}
        />
        <Upgrade
